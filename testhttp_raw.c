...
//  char *host_end = NULL;
  char *host_slash;
  if((host_slash = strchr(host_begin, '/')) != NULL) {
    host_length = host_slash - host_begin;
  }
  char *host_hash;
  if((host_hash = strchr(host_begin, '#')) != NULL) {
    if(host_hash - host_begin < host_length) {
      host_length = host_hash - host_begin;
    }
  }
  char *host_question;
  if((host_question = strchr(host_begin, '?')) != NULL) {
    if(host_question - host_begin < host_length) {
      host_length = host_question - host_begin;
    }
  }
  ...
  if(host_slash || host_hash || host_question) {
    host[host_length] = '\0';
  }
  return host;
}

// Funkcja wyciąga adres zasobu z podanego adresu.
// fetch_get_address alokuje pamięć na adres zasobu i przepisuje go na podstawie adresu oraz hosta podanego w argumencie.
// Jeśli nie udało się zaalokować pamięci, przerywa program z odpowiednim komentarzem wypisywanym na strumień
// błędu.
// Zwraca wskaźnik na linijkę z adresem zasobu.
char *fetch_get_address(const char *address, char *host) {
  char *host_begin = strstr(address, host);
  size_t host_length = strlen(host);
  char *host_end = host_begin + host_length;
...
char *create_get(const char *address, const char *cookies) {
  ...
  char *get_address = fetch_get_address(address, host);
  size_t request_length = 15 + strlen(GET_STRING) + strlen(get_address) + strlen(HTTP_STRING) + strlen(HOST_STRING) + strlen(host) + strlen(COOKIE_STRING) + strlen(cookies) + strlen(CONNECTION_CLOSE_STRING);
  char *result = malloc(request_length * sizeof(char));
  if(!result) {
    fatal("couldn't allocate memory");
  }
  sprintf(result, "%s %s %s\r\n%s %s\r\n%s %s\r\n%s\r\n\r\n", GET_STRING, get_address, HTTP_STRING, HOST_STRING, host,
...
char *fetch_response_cookies_and_length(int socket) {
  ...
  while((response_header_line = read_line(response, &response_header_line_length)) != NULL) {
    if(strncasecmp(response_header_line, "set-cookie: ", 12) == 0) {
      char *cookie_end;
    ...
    if(strcasecmp(response_header_line, "transfer-encoding: chunked\r\n") == 0) {
      chunked = true;
    }
